...
/* Debemos incluir el archivo .h de la biblioteca que estamos definiendo */
#include "AHT10.h"
...
AHT10_DATA AHT10_Read(void){
	...
	AHT10_DATA dataread; // Variable tipo estructura para guardar los datos leídos de temperatura y humedad
	...
	HAL_I2C_Master_Transmit(&hi2c1, I2C_ADDRESS_AHT10<<1, triggerdata, sizeof(triggerdata), 50);
	...
	if(HAL_I2C_Master_Receive(&hi2c1, I2C_ADDRESS_AHT10<<1, bufdata, sizeof(bufdata), 100) == HAL_OK){
		...
		dataread.tcelsius=(temp/1048576.0)*200.0-50.0;
		...
		dataread.hr=(hum/1048576.0)*100.0;
	}
	return dataread;
}
