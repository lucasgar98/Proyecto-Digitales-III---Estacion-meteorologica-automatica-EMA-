...
/* Esta función se utiliza para escribir un registro del BMP180. Se envía la dirección
 * del registro junto con el valor a escribir en el registro */
void BMP180_WriteRegister(uint8_t dirreg,uint8_t valreg){

	uint8_t buf2[2] = {dirreg,valreg}; // Definimos un vector para guardar la dir. y el valor del registro
	HAL_I2C_Master_Transmit(&hi2c2, I2C_ADDRESS_BMP180<<1, buf2, sizeof(buf2), 50);
}

/* Función que se utiliza para leer un registro del BMP180. Se envía primero la dirección del registro y
 * luego se escribe el contenido del registro */
uint8_t BMP180_ReadRegister(uint8_t dir){

	uint8_t buf; // Variable para recibir un byte del sensor (el contenido del registro)
	HAL_I2C_Master_Transmit(&hi2c2, I2C_ADDRESS_BMP180<<1, &dir, sizeof(dir), 50);
	HAL_I2C_Master_Receive(&hi2c2, I2C_ADDRESS_BMP180<<1, &buf, sizeof(buf), 50);
	return buf;
}

/* Rutina que obtiene los coeficientes de calibración que están almacenados en la memoria EEPROM del
 * BMP180. Devuelve un tipo de dato short. */
short BMP180_Get_CalCoef1(uint8_t DIRMSB,uint8_t DIRLSB){
	...
	/* Mediante el operador de desplazamiento y el operador OR a nivel de bits obtenemos el coeficiente
	 * de calibración */
	calcoef=(MSB<<8)|LSB;
...
/* Función idéntica a la anterior, con la diferencia de que devuelve un tipo de dato "unsigned short" */
...
/* Rutina que obtiene todos los coeficientes de calibración y los guarda en una estructura */
...
/* Rutina que obtiene el dato en crudo de temperatura */
long BMP180_Get_RawTemperature(void){
	...
	long UT; // Dato en crudo de temperatura
	...
	/* Obtenemos el dato de temperatura desplazando el MSB 8 posiciones a la izquierda y uniéndolo con
	 * el LSB */
...
/* Rutina que obtiene el dato en crudo de presión, según el modo de conversión seleccionado */
long BMP180_Get_RawPressure(BMP180_OSS OSS){
	...
	long UP; // Variable para guardar el dato de presión en crudo
...
/*	 El valor a escribir en el registro de control dependerá del modo de precisión en la conversión de la
	...
	/* Obtenemos el dato de presión mediante desplazamiento de bits */
...
/* Rutina que implementa el algoritmo para obtener el dato real de presión atmosférica a partir de los
 * coeficientes de calibración y los datos en crudo de temperatura y presión */
float BMP180_Get_TruePressure(BMP180_OSS oss){
	...
	/* Realizamos los cálculos auxiliares que figuran en la hoja de datos del sensor para obtener el dato
	 * real de presión */
